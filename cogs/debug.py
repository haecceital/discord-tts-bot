...
from utils import check_id, format_sec, codeblock
...
class DebugCog(commands.Cog):
    ...
    async def mod(self, ctx, target: str = None, value = None):
        ...
        if target == "timeout":
            ...
            else: await ctx.reply("syntax error")
    ...
    async def query(self, ctx, target: str = None):
        ...
        if target == "queue":
            ...
            result = str(tts_queue.qsize()) + "\n\n"
            for i in range(tts_queue.qsize()):
                tmp = tts_queue._queue[i]
                tmp = tmp.get("path") or tmp.get("text")
                result += (tmp if len(tmp) <= 20 else tmp[:20] + f"... {len(tmp) - 20}") + '\n'
