...
class AdminCog(commands.Cog):
    ...
    async def dump(self, ctx, *, s: str = None):
        ...
        if execute:
            ...
            target_str = (
                "" if "object at 0x" in str(target) else "obj: " + str(target) + "\n\n"
            )
            target_normal_atts = (
                "attributes: "
                + ", ".join(
                    [
                        att
                        for att in dir(target)
                        if not att.startswith("_") and name in att
                    ]
                )
                + "\n\n"
            )
            ...
            result = codeblock(
                f"{target_class}{target_str}{target_normal_atts}{target_protected_atts}"
            )
            if len(result) > 2000:
                result = result[:1997] + "```"
    ...
    async def exec(self, ctx, *, cmd: str = None):
        ...
        env = {
            ...
            "guild": ctx.guild,
            "runtime": runtime,
            "saved_obj": runtime.saved_obj,
        }
        to_compile = f"async def _eval_job():\n" + "\n".join(
        ...
        try:
            ...
            await ctx.reply(
                codeblock(f"compile failed:\n{traceback.format_exc()}", "py")
            )
            return

        _eval_job = env["_eval_job"]

        try:
            with contextlib.redirect_stdout(stdout):
                result = await _eval_job()
        except Exception as e:
            await ctx.reply(
                codeblock(f"runtime error:\n{traceback.format_exc()}", "py")
            )
            return
